...
(
	...
	(customer_id),
    CONSTRAINT CHK_Customer_gender check
	(gender IN
	(N'Nam', N'Nữ')),
    CONSTRAINT CHK_Customer_type check
	(type IN
	(N'cá nhân', N'doanh nghiệp')),
	...
	CONSTRAINT CHK_Account_status CHECK
		...
		(
			...
			-- Không dùng FK trỏ tới bảng nào hết, vì có 2 bảng nhân viên và khách hàng -> Xử lý việc này bằng java.
...
foreign key (branch_id) references branch(branch_id);
		...
		-- -------- branch ----------
		INSERT INTO Branch
			(branch_id, branch_name, address, status)
		VALUES
		...
		INSERT INTO Employee
			(employee_id, full_name, branch_id, role, status, created_at)
		VALUES
		...
		INSERT INTO Collateral
			(collateral_id, owner_id, asset_type, estimated_value, currency, status)
		VALUES
		...
		INSERT INTO Transaction
			(transaction_id, from_account, to_account, transaction_type, amount, currency, date, status)
		VALUES
		...
		-- -------- loan ------------
		INSERT INTO Loan
			(loan_id, customer_id, loan_type, amount, currency, interest_rate, start_date, end_date, status, collateral_id)
		VALUES
		...
		INSERT INTO user_account
			(username, password, user_type, reference_id)
		VALUES
